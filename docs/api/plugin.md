---
title: "Plugin System API Reference"
description: "Custom type handler registry for extending the type system. int argparse_register_type(const char *type_name, Register a custom type handler globally. Returns 0 on success, -1 on failure. Maximum 32 registered types. Registering a duplicate name replaces the existing handler. void argparse_unregister_type(const char *type_name); Remove a custom type handler. No-op if the type is not registered. typedef bool (*argparse_type_parse_fn)(const char *value, void *output, Parse callback for custom types. Returns `true` on success, `false` on error with message in `error_buf`. Base value for custom type identifiers. Custom types should use IDs >= 1000 to avoid conflicts with built-in types."
---
